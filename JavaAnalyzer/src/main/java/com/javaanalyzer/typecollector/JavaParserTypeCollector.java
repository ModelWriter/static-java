...
public class JavaParserTypeCollector {

    private String path;
    private CombinedTypeSolver combinedTypeSolver;

    public JavaParserTypeCollector(String path) {
        this.path = path;
    ...
    public Set<Type> collect() {
                        ...
                        resolved.getAncestors().stream()
                                ...
                                .forEach(superType -> {
                                    if (superType instanceof InterfaceType)
                                        ((InterfaceType) superType).addSubType(((InterfaceType) type));
                                });
                    }
                        ...
                        resolved.getAncestors().stream()
                                ...
                                .forEach(superType -> {
                                    if (superType instanceof ClassType)
                                        ((ClassType) superType).addSubType(((ClassType) superType));
                                    else if (superType instanceof InterfaceType)
                                        ((ClassType) type).addImplementedType(((InterfaceType) superType));
                                });
                    }
        ...
        return typeMap.values().stream().distinct().collect(Collectors.toSet());
    }
}
